...
1. ~~**Design polish**~~ **✅ DONE.** Logo colors used throughout; flagship Apply treatment propagated to all 3.
   ...
   **Mobile QA pass:** added hamburger nav on all 3 (was NONE <940px — critical); cleaned mobile header (tagline
   hidden, compact CTA, no wrap); Apply form shows first on mobile; removed ALL visible `[to confirm]`/`[disclosures]`
   bracket placeholders from footers (clean truthful compliance lines). Verified live on mobile. CSS: Mortgage v11,
   Premium/Finance v9.
2. **Content/compliance pass (CLIENT-PROVIDE — can't finish without them):** NMLS #71517 active status, real LA OFI
   license #s (removed the placeholder brackets; add real #s at launch), real office hours (softened to "call us"),
   real staff headshots (initial-avatars now), and a real office/building photo (using honest area photos now).
